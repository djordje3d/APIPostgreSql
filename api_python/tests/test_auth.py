...
def test_protected_endpoint_returns_401_with_expired_token(
    ...
    monkeypatch.setattr("api_python.app.auth_jwt.JWT_EXPIRE_MINUTES", -1)  # expire = now - 1 minute
    token = create_token("testuser")
    response = client.get("/garages", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 401
...
def login_configured(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("api_python.app.routers.auth.AUTH_USERNAME", "testuser")
    monkeypatch.setattr("api_python.app.routers.auth.AUTH_PASSWORD", "testpass")
...
def test_login_returns_503_when_not_configured(
    ...
    monkeypatch.setattr("api_python.app.routers.auth.AUTH_USERNAME", None)
    response = client.post(
        "/auth/login",
