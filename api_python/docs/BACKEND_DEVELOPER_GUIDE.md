...
The backend follows a layered structure in `api_python/app/` (or `app/` when your current directory is `api_python/`):
...
This project uses Alembic for schema evolution in `api_python/alembic/versions/`.
...
2. Set `sqlalchemy.url` in `api_python/alembic.ini` to the same DB used by app `DATABASE_URL`.
...
```bash
alembic -c api_python/alembic.ini revision -m "describe_change"
```

Then implement `upgrade()`/`downgrade()` in the new revision file and run `alembic -c api_python/alembic.ini upgrade head` locally.

Tip: keep migrations small and focused, one logical schema change per revision when possible.

## 5) Test strategy

Current tests are integration-style and live in `api_python/tests/`.
...
```bash
pytest api_python/tests/test_health.py -v
```
...
```bash
uvicorn api_python.app.main:app --reload --host 0.0.0.0 --port 8000
```
