...
- Backend login configured (`AUTH_USERNAME` / `AUTH_PASSWORD` or `AUTH_PASSWORD_HASH` in `api_python/.env`, with legacy root `.env` fallback) — the dashboard always routes through `/login` until a JWT is stored
...
1. Optional: copy `env.example` to `.env` in this folder. If you skip it, defaults below apply.
   ...
   **Dev proxy (optional):** Vite can proxy `/api` to the backend (`vite.config.ts`). To use it, set `VITE_API_URL` to the dashboard origin with the `/api` prefix, e.g. `http://localhost:5173/api` (no trailing slash). Otherwise point `VITE_API_URL` directly at the API.
...
- **Login fails / "not configured":** Set `AUTH_USERNAME` and `AUTH_PASSWORD` (or `AUTH_PASSWORD_HASH`) in `api_python/.env` (preferred; legacy root `.env` fallback is supported) and restart the API.
